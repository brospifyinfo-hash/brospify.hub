...
// preview with optional manual text editing → copy the Subject &
// Liquid blocks into Shopify's notification editor. The legacy grid
// + per-template subroute is gone: this is the entire feature on one
// screen. We removed the in-app "1-click deploy" — customers now
// copy the generated code themselves and paste it into their own
// shop, which keeps the hub free of Shopify-app-only surfaces.
          ...
          <section className="col-span-12 lg:col-span-6 space-y-4 sm:space-y-5">
            ...
            {generated ? (
              <>
                ...
                {/* Code zum Kopieren — copy-into-Shopify replacement
                    for the old 1-click deploy. Two prominent blocks:
                    Subject + Liquid. The user pastes them straight
                    into Shopify → Einstellungen → Benachrichtigungen
                    → {tpl.shopifyName}. */}
                <div className="glass-email p-5 sm:p-6 space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-[13px] text-white/50 mt-1 leading-relaxed">
                        ...
                        <span className="text-white/75">Einstellungen → Benachrichtigungen → </span>
                        <code className="font-mono text-white/80 text-[12px]">{tpl.shopifyName}</code> ein.
                      </p>
                    </div>
                  </div>
              ...
              <div className="glass-email p-5 space-y-3">
                ...
                <ul className="space-y-3 text-[12px] text-white/60 leading-relaxed">
                  ...
                  <li>
                    ...
                    Kopiere am Ende{" "}
                    <em className="not-italic text-[#95BF47]">Betreff</em> und{" "}
                    <em className="not-italic text-[#95BF47]">HTML-Code</em>{" "}
                    und füge sie in Shopify unter{" "}
                    <span className="text-white/60">Einstellungen → Benachrichtigungen</span>{" "}
                    ein.
                  </li>
                </ul>
              </div>
                  ...
                  <ol className="space-y-2.5 text-[12.5px] text-white/65 leading-relaxed">
                    ...
                    <li>
                      ...
                      <span className="block text-white/55 mt-0.5">
                        Einstellungen → Benachrichtigungen → {tpl.shopifyName}
                      </span>{" "}
                      ein.
                    </li>
                  </ol>
...
// ─── Copy code block ─────────────────────────────────────────────
// Shared component for the "Code zum Kopieren" section. Shows the
// value in a code surface with a prominent copy button. Single-line
// values get a one-line input style; multi-line values get a fixed-
// height <pre> with overflow scroll.
...
function CopyCodeBlock({
  ...
  return (
    <div className="space-y-2">
        ...
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-semibold border transition ${
            copied
              ...
              : "bg-[#95BF47] border-[#95BF47] text-black hover:brightness-110"
          }`}
        >
          {copied ? (
            <>
      ...
      {multiline ? (
        <pre className="bg-black/40 border border-white/[0.06] rounded-xl p-3 sm:p-4 overflow-x-auto text-[11.5px] leading-relaxed font-mono text-white/75 max-h-[320px] overflow-y-auto whitespace-pre-wrap break-words">
          {value}
        </pre>
      ) : (
        <div className="bg-black/40 border border-white/[0.06] rounded-xl px-3 py-2.5 font-mono text-[12.5px] text-white/85 break-all">
          {value}
        </div>
      )}
    </div>
