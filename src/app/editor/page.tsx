  ...
  const startOwnGenesis = useCallback((r: IntakeResult) => {
    ...
    setGenesisJob({
      ...
      prepare: async () => {
        ...
        // neutrale Flächen statt Farbe) direkt im User-Prompt.
...
MANDATORY for this full build: (1) fonts and accent derived from the product photos — the page itself stays all white (or all black if the niche clearly calls for it), colour only on accents, buttons and icons; (2) a COMPLETE page of 8–10 sections: EXACTLY ONE hero at the top, then features/benefits, story/imagery, reviews in the lower half, FAQ near the end, guarantee + closing CTA — never two similar sections or two large image sections stacked, but do cover the full funnel — benefits, proof, objections, guarantee and a closing CTA all belong on the page; (3) EVERY section fully copywritten for this exact product; (4) FULLY personalize the buy box: product-specific copy for every visible block (sale banner, urgency, benefits, stock, bundles, free gift, delivery), 4 niche-matching benefit icons, plus 2–3 fitting trust blocks; (5) section surfaces stay neutral — at most two sections get a subtle grey/near-black surface, no gradients, no shape edges.`
...
PFLICHT für diesen Neubau: (1) Schriften und Akzentfarbe aus den Produktfotos ableiten — die Seite selbst bleibt komplett weiß (oder komplett schwarz, wenn die Nische das klar verlangt), Farbe nur auf Akzenten, Buttons und Icons; (2) eine VOLLSTÄNDIGE Seite mit 8–10 Sections: GENAU EIN Hero oben, danach Features/Vorteile, Story/Bilder, Bewertungen in der unteren Hälfte, FAQ weit unten, Garantie + Abschluss-CTA — nie zwei ähnliche Sections oder zwei große Bild-Sections direkt untereinander, aber der Funnel muss vollständig sein — Vorteile, Beweise, Einwände, Garantie und Abschluss-CTA gehören alle auf die Seite; (3) JEDE Section vollständig auf genau dieses Produkt getextet; (4) Die KAUFBOX VOLL personalisieren: produktspezifische Texte für jeden sichtbaren Baustein (Sale-Banner, Dringlichkeit, Vorteile, Lager, Bundles, Gratis-Geschenk, Lieferzeit), 4 nischen-passende Vorteil-Icons, dazu 2–3 passende Vertrauens-Bausteine; (5) Section-Flächen bleiben neutral — höchstens zwei Sections bekommen eine dezente graue bzw. fast schwarze Fläche, keine Verläufe, keine Formen-Kanten.`;
