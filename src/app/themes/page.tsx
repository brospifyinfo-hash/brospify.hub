...
export default function ThemesPage() {
  ...
  return (
    <div className="min-h-screen bg-mesh">
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-6 py-3 sm:py-8 space-y-3 sm:space-y-5">
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <h1 className="text-base sm:text-3xl font-bold flex items-center gap-2">
                <Palette className="w-5 h-5 sm:w-7 sm:h-7 text-purple-400 shrink-0" />
            ...
            {settings.themeVersion && (
              <div className="text-right shrink-0">
                <div className="text-[8px] sm:text-[10px] uppercase tracking-widest text-zinc-500 font-semibold">Version</div>
                <div className="text-sm sm:text-lg font-bold text-[#95BF47] tabular-nums">{settings.themeVersion}</div>
              </div>
            )}
          </div>
        </motion.div>
        ...
        <AnimatePresence>
          {pushResult && (
            ...
            >
              ...
              <p className="text-xs sm:text-sm flex-1">{pushResult.message}</p>
        ...
        <motion.div
          ...
          className="glass-strong rounded-xl sm:rounded-2xl p-3 sm:p-6 border border-white/10"
        >
          <div className="flex items-start gap-3 sm:gap-5">
            <div className="w-12 h-12 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center shrink-0">
              <FileArchive className="w-6 h-6 sm:w-10 sm:h-10 text-purple-400" />
            </div>

            <div className="flex-1 min-w-0">
              <h2 className="text-base sm:text-2xl font-bold leading-tight truncate">
                {settings.themeFileName || "Brospify Theme"}
              </h2>
              <p className="text-zinc-400 text-[11px] sm:text-sm mt-1 leading-snug">
          ...
          {/* Actions */}
          <div className="mt-3 sm:mt-5 flex flex-col sm:flex-row gap-2">
            {settings.themeFileUrl ? (
              <a
                href={settings.themeFileUrl}
                download
                className="btn-accent flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:px-6 sm:py-3 rounded-xl text-xs sm:text-sm font-semibold"
              >
                <Download className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                Theme herunterladen
              </a>
            ) : (
              <div className="glass rounded-xl px-4 py-2.5 sm:px-6 sm:py-3 text-zinc-500 text-xs sm:text-sm flex items-center justify-center gap-2">
            ...
            {session.hasShopifyConnection && settings.themeFileUrl && (
              <button
                ...
                disabled={pushing}
                className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:px-6 sm:py-3 rounded-xl text-xs sm:text-sm font-semibold border border-purple-500/30 bg-purple-500/10 text-purple-300 hover:bg-purple-500/20 transition disabled:opacity-70"
              >
                {pushing ? (
                  <>
          ...
          {pushing && (
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-zinc-500 text-[10px] sm:text-xs mt-2"
            >
        ...
        {settings.themeChangelog && (
          ...
          >
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2.5 sm:px-4 sm:py-3 text-left"
            >
              <Info className="w-3.5 h-3.5 text-[#95BF47] shrink-0" />
              <span className="text-xs sm:text-sm font-semibold flex-1">Changelog</span>
            ...
            <AnimatePresence>
              {showChangelog && (
                ...
                >
                  <pre className="text-[11px] sm:text-sm text-zinc-400 whitespace-pre-wrap font-sans leading-relaxed px-3 sm:px-4 pb-3 sm:pb-4 pt-1 border-t border-white/[0.04]">
                    {settings.themeChangelog}
                  </pre>
                </motion.div>
              )}
        ...
        {!session.hasShopifyConnection && (
          <motion.div
            ...
            className="rounded-xl sm:rounded-2xl border border-[#95BF47]/20 bg-[#95BF47]/[0.04] p-3 sm:p-4"
          >
            <div className="flex items-start sm:items-center gap-2.5 sm:gap-3">
              <Store className="w-5 h-5 sm:w-7 sm:h-7 text-[#95BF47] shrink-0 mt-0.5 sm:mt-0" />
              <div className="flex-1 min-w-0">
                <h3 className="text-xs sm:text-sm font-bold mb-0.5">Shopify verbinden für 1-Klick-Push</h3>
                <p className="text-[10px] sm:text-xs text-zinc-500 leading-snug">
                  Spar dir den manuellen Theme-Upload — verbinde deinen Shop einmal, push immer mit einem Klick.
                </p>
              </div>
              <button
                onClick={() => router.push("/setup")}
                className="btn-accent px-3 py-1.5 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl text-[11px] sm:text-sm font-semibold shrink-0 flex items-center gap-1"
              >
                Setup
