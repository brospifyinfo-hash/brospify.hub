...
interface TourStep {
  ...
  color: string;
  bg: string;
  titleKey: keyof typeof import("@/lib/dictionaries/de.json")["tour"];
  descKey: keyof typeof import("@/lib/dictionaries/de.json")["tour"];
    ...
    <div className="fixed inset-0 z-[100]">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={`${
            spotlightRect && current.position === "bottom"
              ? ""
              : "fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
          } z-[101] w-[90vw] max-w-md`}
        ...
        >
          <div className="bg-zinc-900/95 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div
                className={`w-12 h-12 rounded-xl ${current.bg} border border-white/10 flex items-center justify-center shrink-0`}
              >
                <current.icon className={`w-6 h-6 ${current.color}`} />
              </div>
              <div>
                <h3 className="text-lg font-bold">
                  {t.tour[current.titleKey as keyof typeof t.tour]}
                </h3>
                <p className="text-sm text-zinc-400 mt-0.5">
                  {t.tour[current.descKey as keyof typeof t.tour]}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-5">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={handleSkip}
                  className="text-xs text-zinc-600 hover:text-zinc-400 transition px-3 py-1.5"
                >
                  {t.tour.skip}
                </button>
                <button
