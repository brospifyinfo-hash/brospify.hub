...
- NEU: Auf [Theme](/themes) kann man im "Fertiges Produkt-Theme"-Bereich einen STIL wählen (Modern, Elegant, Bold, Verspielt), dann Farben (5 Rollen), Überschriften-/Text-Schrift und Ecken (Kantig/Leicht/Rund) anpassen. Eine Live-Vorschau zeigt die OBERSEITE der Produktseite (Galerie + Infospalte: Dringlichkeit, Titel, Bewertung, Vorteile, Lager, Preis, Bundle-Auswahl, Kaufen-Button, Zahlarten, Gratis-Geschenk, Liefer-Timeline — bis VOR der Beschreibung) und aktualisiert sich SOFORT bei jeder Einstellungsänderung. Leere Texte sind mit Beispiel-Verkaufstexten gefüllt (nie rohe Platzhalter). Der Download liefert dann das vollständige Shopify-Theme (Start- + Produktseite, alle Sections) mit Stil und gewählten Einstellungen.
...
- Basis des Editors ist automatisch das NEUESTE vom Admin hochgeladene Theme (Admin: /admin/themes). Lädt der Admin ein neues Theme hoch, baut der Editor ab sofort auf diesem auf — der Kunde merkt davon nichts, es funktioniert direkt. Ist noch kein Theme hochgeladen, nutzt der Editor die eingebaute Brospify-Schablone. Farben, Schriften und Ecken werden in jedes hochgeladene (Dawn-basierte) Theme zuverlässig eingesetzt; KI-Produkttexte werden dort eingesetzt, wo das Theme die passenden Platzhalter hat.
