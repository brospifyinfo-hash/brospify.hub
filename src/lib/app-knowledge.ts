...
- NEU: Auf [Theme](/themes) kann man im "Fertiges Produkt-Theme"-Bereich einen STIL wählen (Modern, Elegant, Bold, Verspielt), dann Farben (5 Rollen), Überschriften-/Text-Schrift und Ecken (Kantig/Leicht/Rund) anpassen. Eine Live-Vorschau zeigt die ECHTE Produktseite 1:1 aus dem Theme (serverseitig gerendert) und aktualisiert sich bei jeder Einstellungsänderung — sie entspricht dem, was der Download liefert. Leere Texte sind mit Beispiel-Verkaufstexten gefüllt (nie rohe Platzhalter). Der Download liefert dann das vollständige Shopify-Theme (Start- + Produktseite, alle Sections) mit Stil und gewählten Einstellungen.
