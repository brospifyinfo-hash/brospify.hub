...
- LIVE-SYNC (dynamische Buy Box): Der Bereich "Produktinformationen" (Titel, Bewertung, Vorteile, Preis, Varianten, Mengenrabatte, Kaufen-Button, Zahlarten, Liefer-Timeline) wird NICHT mehr fest ins ZIP geschrieben, sondern lädt sich im Shop LIVE vom Hub. Im Theme steckt dafür nur ein Block "Brospify Dynamic Buybox" mit einem Sync-Code (kommt automatisch vorbefüllt mit dem Download — null Einrichtung). Vorteil: Ändert der Kunde das Design im [Theme-Editor](/themes/editor) und klickt "Live aktualisieren", erscheint das neue Design in wenigen Minuten in ALLEN Shops mit diesem Code — ohne neues ZIP, ohne Shopify anzufassen. Der Kauf läuft trotzdem 100% über Shopifys eigene Warenkorb-Logik im Shop des Kunden (der Hub ist am Kauf nie beteiligt). Ist der Hub mal nicht erreichbar, zeigt der Shop das zuletzt geladene Design (Browser-Cache) oder eine einfache eingebaute Kauf-Form — kaufbar bleibt der Shop IMMER. Beim Laden gibt es einen dezenten Skeleton-Loader. Galerie (Produktbilder) und alle Sektionen unter der Kaufbox bleiben fest im ZIP. "Live aktualisieren" ohne vorherigen Download → Hinweis, dass zuerst einmal heruntergeladen werden muss (der Download erstellt den Code).
