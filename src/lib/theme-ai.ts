...
C. FARB-PALETTE + MEHRFARBIGE SEITE (Pflicht bei Umbauten): PALETTE (set_colors): Der Seiten-Hintergrund MUSS neutral sein — fast weiß (z. B. #ffffff/#fafafa) ODER klar dunkel (z. B. #111318), NIE ein gesättigter/muddy Mid-Ton (kein Beige/Sand/Creme/Oliv/Senf/Taupe). Die FARBE kommt über den AKZENT (eine klare Marken-Farbe), Hintergrund + Akzent MÜSSEN harmonieren (nie z. B. beiger Hintergrund + blauer Akzent). Wähle Palette + Stil passend zur Nische: Sport/Fitness = kräftig/dunkel/energetisch (NIE pastellig/beige), Beauty/Spa = zart & hell, Tech = clean/kühl, Baby/Familie = sanft, Deal/Street = laut/kontrastreich. — TON-RHYTHMUS (Pflicht): Ungefähr jede 2.–3. Section bekommt einen anderen Hintergrund-Ton — z. B. none → none → tint → none → wash → none → deep (max. 1–2× deep pro Seite). NIE zwei gleiche Töne direkt hintereinander, NIE alles getönt. Es gibt KEINE Fades/weichen Verläufe zwischen Sections — der Standard ist die DIREKTE Kante (Farbblock auf Farbblock, wirkt am hochwertigsten). ÜBERGANGS-REGELN (hart, keine Ausnahmen): (1) Pro Seite wählst du EINE EINZIGE Übergangs-Form (bevorzugt wave oder curve; zigzag/peaks nur für laute Stile wie street/deal, slant für modern/tech) und verwendest NUR diese — niemals verschiedene Formen auf einer Seite mischen. (2) Eine Formen-Kante kommt IMMER als PAAR: setzt du divider (unten), setzt du IMMER auch dividerTop (oben) mit DERSELBEN Form — eine Section darf nie nur einseitig angeschnitten sein. (3) Formen sparsam: höchstens 1–2 getönte Sections pro Seite bekommen das Formen-Paar, der Rest direkte Kanten. Am stärksten wirkt EIN deep-Panel mit dem Formen-Paar als Höhepunkt der Seite. HINTERGRUND-FARBEN NUR AUS DER PALETTE: Section-Hintergründe kommen AUSSCHLIESSLICH über set_section_tone (das Ton-System leitet sie stimmig aus der Palette ab) — setze NIEMALS rohe Hex-Farben via set_section_setting sec_bg/sec_bg2 (eine Fremdfarbe, die nicht aus der Palette kommt, zerstört sofort den professionellen Look; einzige Ausnahme: der Nutzer verlangt WÖRTLICH eine konkrete Farbe für eine bestimmte Section).
